...
%  date: 17.11.2016
%
%  modified version for tangent distance, Samira Kabri
  ...
  methods
    ...
    function obj = kNNGraph(varargin)
      ...
      elseif strcmp(data.type,'point_cloud') % point cloud data
        ...
        % check which implementation is specified for building the kdtree
        if strcmp(implementation, 'cpp')
          
          % generate a kd-tree from given points using C++ libary
...
%% for BOTH        
        ...
        for u = 1:obj.numberOfVertices
          ...
          if strcmp(implementation, 'cpp')
            ...
            indices = kdtree.knn( data.coordinates(u,:), neighborhood.numberOfNeighbors+1);
            
            % delete center node itself from neighborhood
            indices = setdiff(indices,u);
...
%% Include following lines for EUCLIDEAN DISTANCE
          ...
          distances = distanceFunction.outerNorm(sum(distanceFunction.innerNorm(distances),2));
...
%% for BOTH          
    ...
    % for symmetrized kNN these functions are not correct
    
%     % gets the neighbor indices for a given node u using the clear
%     % structure due to fixed amount of neighbors
%     function neighbor_indices = getNeighbors(obj,node_index)
%       k = obj.graphProperties.neighborhood.numberOfNeighbors;
%       neighbor_indices = obj.edges(k*(node_index-1)+1:node_index*k,2);
%     end
    
%     % gets the weights of neighbors for a given node u using the clear
%     % structure due to fixed amount of neighbors
%     function weights = getWeights(obj,node_index)
%       k = obj.graphProperties.neighborhood.numberOfNeighbors;
%       weights = obj.edgeWeights(k*(node_index-1)+1:node_index*k);
%     end
